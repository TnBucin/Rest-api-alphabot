...
const listkey = ["RXYBOTZ", "RaditXy7"];
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'RXYBOTZ') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'RXYBOTZ')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/RaditXyofc/Asupan/main/cecan/vietnam.json`))
